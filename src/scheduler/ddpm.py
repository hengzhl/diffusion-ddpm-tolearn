...
class DDPMPipeline:
    ...
    def sampling(self, model, initial_noise, device, save_all_steps=False):
        ...
        for timestep in tqdm(range(self.num_timesteps - 1, -1, -1)):
            ...
            ts = timestep * torch.ones(image.shape[0], dtype=torch.long, device=device) # 时间编码
            predicted_noise = model(image, ts)
            beta_t = self.betas.to(device)[timestep]
            alpha_t = self.alphas.to(device)[timestep]
            alpha_hat = self.alphas_hat.to(device)[timestep]
            ...
            alpha_hat_prev = self.alphas_hat.to(device)[timestep - 1]
